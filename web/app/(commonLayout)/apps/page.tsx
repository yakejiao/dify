...
type AppListFooterProps = {
  isVisible: boolean
}

const AppListFooter: React.FC<AppListFooterProps> = async ({ isVisible }) => {
  ...
  return (
    <>
      {isVisible
      && <div><h3 className='text-xl font-semibold leading-tight text-gradient'>{t('join')}</h3>
  ...
  return (
    <div className='relative flex flex-col overflow-y-auto bg-gray-100 shrink-0 h-0 grow'>
      ...
      <footer className='px-12 py-6 grow-0 shrink-0'>
        <AppListFooter isVisible={!PLATFORM_CUSTORM.open} />
      </footer>
    </div >
  )
}
